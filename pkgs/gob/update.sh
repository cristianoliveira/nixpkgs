...
darwin_aarch64_url="${base_url}/gob_${version}_darwin_arm64.tar.gz"
darwin_amd64_url="${base_url}/gob_${version}_darwin_amd64.tar.gz"
linux_aarch64_url="${base_url}/gob_${version}_linux_arm64.tar.gz"
linux_amd64_url="${base_url}/gob_${version}_linux_amd64.tar.gz"
...
# Replace the 4 sha256 literals in order: darwin aarch64, darwin amd64, linux aarch64, linux amd64
sha_re = re.compile(r'(?m)^(\s*if pkgs\.stdenv\.isAarch64 then )"sha256-[^"]+"(\s*)$')
